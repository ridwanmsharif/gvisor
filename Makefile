...
GPG_TEST_OPTIONS := $(shell if gpg --pinentry-mode loopback --version >/dev/null 2>&1; then echo --pinentry-mode loopback; else echo; fi)
$(RELEASE_KEY):
	...
	gpg --batch $(GPG_TEST_OPTIONS) --export-secret-keys --no-default-keyring --keyring $$T > $@; \
	rc=$$?; rm -f $$T $$C; exit $$rc
