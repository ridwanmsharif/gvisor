...
TEST(FlockTestNoFixture, FlockEpoll) {
  auto fd = ASSERT_NO_ERRNO_AND_VALUE(NewEpollFD());
  EXPECT_THAT(flock(fd.get(), LOCK_EX | LOCK_NB), SyscallSucceeds());
}
...
TEST(FlockTestNoFixture, FlockTimer) {
  int fd;
  ASSERT_THAT(fd = timerfd_create(CLOCK_MONOTONIC, 0), SyscallSucceeds());
  EXPECT_THAT(flock(fd, LOCK_EX | LOCK_NB), SyscallSucceeds());
  EXPECT_THAT(close(fd), SyscallSucceeds());
}

TEST(FlockTestNoFixture, FlockEventFD) {
  auto fd = ASSERT_NO_ERRNO_AND_VALUE(NewEventFD(0, 0));
  EXPECT_THAT(flock(fd.get(), LOCK_EX | LOCK_NB), SyscallSucceeds());
}
